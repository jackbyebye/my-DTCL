...
if __name__ == '__main__':
    ...
    '''dataset setting'''
    config_dtcr['input_length'] = x_train.shape[1]
    config_dtcr['training_samples_num'] = x_train.shape[0]
    config_dtcr['cluster_num'] = len(np.unique(y_train))

    ''' Network config searching. '''
    for encoder_config in [[50,30,30]]:#[100,50,50], [50,30,30]
        config_dtcr['encoder_hidden_units'] = encoder_config

        for lambda_1 in [1e-1]:#1, 1e-1, 1e-2, 1e-3
            ...
            best, best_epoch, train_list, test_list = DTCR_model.train(x_train,y_train,x_train,y_test,bs=4)
